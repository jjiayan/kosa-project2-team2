...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  .nav-root .menu a:hover,
  .nav-root .mobile-menu a:hover{
  ...
  .nav-root .login, .nav-root .userbox{
    width:100%;
    display:flex;
    align-items:center;
    justify-content:flex-end;
  ...
  @media (max-width:768px){
    ...
    .nav-root .hello{ max-width:90px; }
    .nav-root .right{ width:auto; }
  }
</style>
</head>
<body>
...
<header class="nav-root">
    ...
    <div class="center">
        <nav class="menu">
            <a href="${ctx}/roomlist.room?userId=${sessionScope.LOGIN_USER.user_id}">
                <i class="fa-regular fa-comments"></i>스터디
            </a>
            <a href="${ctx}/certificationList.cert">
                <i class="fa-regular fa-calendar"></i>자격증
            </a>
            <a href="${ctx}/adminNotice.admin">
            ...
            <c:when test="${sessionScope.LOGIN_USER.user_status eq 'ADMIN'}">
                <a href="${ctx}/adminMember.admin" 
                   style="display:flex;align-items:center;gap:10px;text-decoration:none;color:inherit;">
                    <img class="avatar" src="${navPhotoUrl}" alt="avatar" 
...
<script> function syncBarcodeToLogo() { const logo = document.getElementById('logoArea'); const barcode = document.querySelector('.barcode'); if (logo && barcode) barcode.style.width = logo.offsetWidth + 'px'; } window.addEventListener('load', syncBarcodeToLogo); window.addEventListener('resize', () => { clearTimeout(window.__resizeTimer); window.__resizeTimer = setTimeout(syncBarcodeToLogo, 100); }); function toggleMenu() { const m = document.getElementById('mobileMenu'); const opened = m.style.display === 'flex'; m.style.display = opened ? 'none' : 'flex'; m.setAttribute('aria-hidden', opened ? 'true' : 'false'); } const mq = window.matchMedia('(min-width: 769px)'); mq.addEventListener('change', e => { if (e.matches) { const m = document.getElementById('mobileMenu'); if (m) m.style.display = 'none'; } }); </script>
