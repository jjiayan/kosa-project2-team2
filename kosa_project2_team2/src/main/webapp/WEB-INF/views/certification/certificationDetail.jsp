...
/* Tabs / year */
.topline{display:flex;align-items:center;justify-content:space-between;margin:24px 0 8px;gap:12px}
...
.year-select{display:flex;align-items:center;gap:10px;color:var(--ink2);font-weight:600}
.year-select select{
  appearance:none;-webkit-appearance:none;-moz-appearance:none;
  border:1px solid #e0e0e0;background:#fff;padding:8px 36px 8px 12px;border-radius:8px;font-weight:600;
...
/* Section */
.section{background:var(--soft);border:1px solid #f0f0f0;border-radius:16px;padding:24px;box-shadow:var(--shadow)}
.section + .section{margin-top:40px;border-top:2px solid #e8e8e8;padding-top:32px}
.section-header{display:flex;align-items:center;gap:12px;margin-bottom:18px;border-bottom:1px solid #eaeaea;padding-bottom:10px}
.section-header .icon{
  width:36px;height:36px;border-radius:10px;display:grid;place-items:center;background:#f8f8f8;
}
.section-header .icon svg{display:block;width:24px;height:24px}
.section-title{font-size:17px;font-weight:600;color:#64748b}

/* Rows */
.rows{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}
@media (max-width:900px){ .rows{grid-template-columns:1fr} }
.info{background:var(--card);border:1px solid #f0f0f0;border-radius:12px;padding:18px;min-height:72px;
...
.stat-card { background: var(--soft); border: 1px solid #f0f0f0; border-radius: 12px; padding: 20px; box-shadow: var(--shadow-soft); display: flex; flex-direction: column; align-items: flex-start; justify-content: center; min-height: 90px; }
...
.chart-box { background: var(--soft); border: 1px solid #f0f0f0; border-radius: 16px; padding: 24px; box-shadow: var(--shadow); height: 350px;}
...
(function(){
  ...
  // 과거 -> 최신(오래된 연도 먼저), 연도 내 회차 오름차순 정렬
  const filtered = chartData
    .filter(Boolean)
    .sort((a, b) => {
      if (a.year === b.year) return a.implSeq - b.implSeq;
      return a.year - b.year; // 과거 -> 최신
    });

  if (filtered.length === 0) return;

  // X축 라벨: "YYYY N회" (한 줄, 회전 없음)
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: (items) => {
              const lab = items[0].label; // "YYYY N회"
              return lab;
            },
            label: (item) => {
              const val = item.raw;
  ...
  // "최근 2년" 카드 계산용: filtered에서 마지막 2개 연도만 추출
