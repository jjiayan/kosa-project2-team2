...
<head>
  ...
  <style>
    ...
    .post-header{ display:grid; grid-template-columns:1fr 120px 120px 100px; gap:20px; padding:15px 20px; border-bottom:2px solid #333; margin-bottom:5px; font-weight:bold; color:#333; font-size:15px }
    .post-item{ display:grid; grid-template-columns:1fr 120px 120px 100px; gap:20px; padding:15px 20px; border-bottom:1px solid #f0f0f0; cursor:pointer; align-items:center }
    .post-item:hover{ background:#f8f9fa; border-radius:8px }
    ...
    .reply-count{ color:#ff6b9d; font-size:.9em; margin-left:5px }
    ...
    .search-btn{ position:absolute; right:5px; top:50%; transform:translateY(-50%); background:linear-gradient(45deg,#ff6b9d,#ff8a80); color:#fff; border:none; border-radius:50%; width:35px; height:35px; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:14px }
    .write-btn{ background:linear-gradient(45deg,#ff6b9d,#ff8a80); color:#fff; border:none; border-radius:12px; padding:12px 20px; font-size:14px; font-weight:bold; cursor:pointer; box-shadow:0 4px 12px rgba(255,107,157,.3) }
    ...
    .page-btn.active{ background:linear-gradient(45deg,#ff6b9d,#ff8a80); color:#fff }
      ...
      <div class="post-list">
        ...
        <c:forEach var="roomBoard" items="${roomBoardList}">
          <div class="post-item" onclick="location.href='${ctx}/roomboarddetail.room?roomBoardId=${roomBoard.roomBoardId}&userId=${sessionScope.LOGIN_USER.user_id}&roomBoardType=GENERAL'">
            ...
            <div class="post-date-cell"><fmt:formatDate value="${roomBoard.updatedAt}" pattern="yyyy-MM-dd"/></div>
