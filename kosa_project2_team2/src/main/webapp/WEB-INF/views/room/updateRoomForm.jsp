...
<main>
  <div class="form-container">
    ...
    <form action="${ctx}/roomupdate.room" method="post" id="postForm">
      <input type="hidden" name="roomId" value="${updateRoomInfo.roomId}"/>
      <input type="hidden" name="userId" id="userId" value="${sessionScope.LOGIN_USER.user_id}"/>
      <input type="hidden" name="examgb" id="examgb" value="${updateRoomInfo.examgb}"/>
          ...
          <div>
            <select class="form-select" name="region1" id="region1">
              ...
              <c:forEach var="region" items="${mainRegionList}">
                <option value="${region.mainRegionId}" ${updateRoomInfo.regionId == region.mainRegionId ? 'selected' : ''}>${region.mainRegion}</option>
              </c:forEach>
            </select>
          </div>
          <div>
        ...
        <div class="select-row">
          <div>
            <select class="form-select" name="examType" id="examType">
              ...
              <option value="필기" ${updateRoomInfo.examgb == '필기' ? 'selected' : ''}>필기</option>
              <option value="실기" ${updateRoomInfo.examgb == '실기' ? 'selected' : ''}>실기</option>
            </select>
          </div>
          <div>
            <div class="search-container">
              <input type="text" 
                     ...
                     autocomplete="off"
                     value="${updateRoomInfo.totalJmName}"
              ...
              <input type="hidden" name="jmcd" id="certificateCode" value="${updateRoomInfo.jmcd}">
              <input type="hidden" name="year" id="certificateYear" value="${updateRoomInfo.year}">
              <input type="hidden" name="implseq" id="certificateImplseq" value="${updateRoomInfo.implseq}">
            </div>
          </div>
        </div>
        ...
        <div class="select-row">
          <div>
            <select class="form-select" name="maxParticipant">
              ...
              <option value="10" ${updateRoomInfo.maxParticipant == 10 ? 'selected' : ''}>10명</option>
              <option value="20" ${updateRoomInfo.maxParticipant == 20 ? 'selected' : ''}>20명</option>
              <option value="30" ${updateRoomInfo.maxParticipant == 30 ? 'selected' : ''}>30명</option>
              <option value="40" ${updateRoomInfo.maxParticipant == 40 ? 'selected' : ''}>40명</option>
              <option value="50" ${updateRoomInfo.maxParticipant == 50 ? 'selected' : ''}>50명</option>
            </select>
          </div>
        </div>

        <input type="text" class="form-control" placeholder="모임방 제목을 입력하세요" name="title" value="${updateRoomInfo.title}" required/>
      </div>
      ...
      <div class="form-section">
        ...
        <div class="thumbnail-upload ${!empty updateRoomInfo.thumbnailUrl ? 'has-image' : ''}" id="thumbnailArea" onclick="document.getElementById('thumbnailInput').click()">
          <div id="thumbnailText" style="${!empty updateRoomInfo.thumbnailUrl ? 'display:none' : ''}">
            📷 클릭하거나 이미지를 드래그하세요<br/>
            <small style="color:#ccc;font-size:12px">권장 크기: 800x400 (2:1 비율)</small>
          </div>
          <img id="thumbnailPreview" class="thumbnail-preview" src="${!empty updateRoomInfo.thumbnailUrl ? ctx.concat(updateRoomInfo.thumbnailUrl) : ''}" style="${!empty updateRoomInfo.thumbnailUrl ? 'display:block' : 'display:none'}"/>
        </div>
        <input type="file" id="thumbnailInput" name="thumbnail" accept="image/*" style="display:none"/>
        <input type="hidden" id="thumbnailUrl" name="thumbnailUrl" value="${updateRoomInfo.thumbnailUrl}"/>
      </div>

      <!-- 소개 -->
      <div class="form-section">
        <div class="section-title">소개</div>
        <textarea id="summernote" name="content">${updateRoomInfo.content}</textarea>
      </div>
...
<script>
  ...
  $(function(){
    // 기존 지역 정보 로드
    const subRegionId = '${updateRoomInfo.subRegionId}';
    if (subRegionId) {
      loadSubRegions('${updateRoomInfo.regionId}', subRegionId);
    }
    ...
    $('#summernote').summernote({
      ...
      callbacks:{
        onInit:function(){ 
          $('.note-editable').attr({'data-gramm':'false','data-gramm_editor':'false','data-enable-grammarly':'false'});
          var existingContent = `${updateRoomInfo.content}`;
    ...
    // 필기/실기 변경 시 검색어 초기화 및 examgb 업데이트
    $('#examType').on('change', function() {
      const selectedExamgb = $(this).val();
      $('#examgb').val(selectedExamgb); // examgb hidden input 업데이트
  ...
  function loadSubRegions(parentId, selectedId) {
    ...
    $.ajax({
      ...
      success: function(list){
        $.each(list, function(_,item){
          const selected = selectedId && item.subRegionId == selectedId ? 'selected' : '';
          $r2.append('<option value="'+item.subRegionId+'" '+selected+'>'+item.subRegion+'</option>');
        });
      }
    });
  }
