...
public class RoomDao {
    ...
    public int insertRoom(RoomDto insertRoomDto) {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(sql1, new String[]{"room_id"});
            ...
            String sql2 = "INSERT INTO JOIN_ROOM (user_id, room_id, room_tier) VALUES (?, ?, 'LEADER')";
    ...
    public PageResult<RoomDto> getRoomsBySearch(SearchCondition searchCondition){
        ...
        try {
            ...
            sql.append("JOIN CERTIFICATION_MASTER cm ON cm.JMCD = ro.JMCD AND cm.YEAR = ro.YEAR AND cm.IMPLSEQ = ro.IMPLSEQ ");
            sql.append("LEFT JOIN LIKE_ROOM lr ON lr.ROOM_ID = ro.ROOM_ID AND lr.USER_ID = ? "); // ← 공백 추가
            ...
            int paramIndex = 1;
            pstmt.setInt(paramIndex++, 1); // (예시) 로그인 유저ID 대입 위치
            ...
            while(rs.next()) {
                ...
                if(!"RECRUITING".equals(rs.getString("room_status"))) {
                    roomStatus = "마감";
                }
    ...
    public int joinRoom(int userId, int roomId) {
        ...
        String sql = "INSERT INTO JOIN_ROOM (user_id, room_id, room_tier) VALUES (?, ?, 'PENDING')";
        try {
            conn = ConnectionPoolHelper.getConnection();
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, userId);
    ...
    public List<RoomBoardDto> getRoomBoardBySearch(SearchCondition searchCondition, int roomId, String roomBoardType) {
        ...
        if (searchCondition.getKeyword() != null && !searchCondition.getKeyword().trim().isEmpty()) {
            sqlBuilder.append("AND rb.ROOM_BOARD_TITLE LIKE ? ");
        }
    ...
    public RoomBoardDto getRoomBoardDetail(int roomBoardId, int userId) {
        ...
        String sql =
            ...
            "  (SELECT COUNT(*) " +
            "     FROM LIKE_ROOM_BOARD l /* ★ 테이블명 수정 */ " +
            "    WHERE l.ROOM_BOARD_ID = rb.ROOM_BOARD_ID) AS like_count, " +
            "  NVL(lrb.USER_ID, 0)    AS like_status, " +
            "  (SELECT CASE WHEN rb.USER_ID = ? THEN 1 ELSE 0 END FROM DUAL) AS is_my_post " +
            "FROM ROOM_BOARD rb " +
            "JOIN \"USER\" u ON u.USER_ID = rb.USER_ID " +
            "LEFT JOIN LIKE_ROOM_BOARD lrb /* ★ 테이블명 수정 */ " +
            "  ON lrb.USER_ID = ? AND lrb.ROOM_BOARD_ID = rb.ROOM_BOARD_ID " +
            "WHERE rb.ROOM_BOARD_ID = ? AND rb.IS_DELETED = 'N'";

        try {
            conn = ConnectionPoolHelper.getConnection();
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, userId);      // is_my_post 판정용
            pstmt.setInt(2, userId);      // 내가 좋아요 눌렀는지
            pstmt.setInt(3, roomBoardId); // 상세 대상 게시글 ID
    ...
    public RoomBoardDto getUpdateInfoRoomDetail(int roomBoardId, String roomBoardType) {
        ...
        String sql = "SELECT rb.ROOM_BOARD_ID as roomBoardId, rb.ROOM_BOARD_TITLE as roomBoardTitle, " +
                "rb.ROOM_BOARD_CONTENT as roomBoardContent, rb.ROOM_BOARD_TYPE as roomBoardType " +
                "FROM ROOM_BOARD rb WHERE rb.ROOM_BOARD_ID = ? AND rb.ROOM_BOARD_TYPE = ?";
        try {
            conn = ConnectionPoolHelper.getConnection();
            pstmt = conn.prepareStatement(sql);
    ...
    public int updateRoomBoard(RoomBoardDto updateRoomDto) {
        ...
        String sql = "UPDATE room_board SET room_board_title = ?, room_board_content = ?, updated_at = SYSDATE WHERE room_board_id = ?";
        try {
            conn = ConnectionPoolHelper.getConnection();
            pstmt = conn.prepareStatement(sql);
    ...
    public int deleteRoomBoard(int roomBoardId) {
        ...
        String sql = "UPDATE room_board SET IS_DELETED = 'Y' WHERE room_board_id = ?";
        try {
            conn = ConnectionPoolHelper.getConnection();
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, roomBoardId);
    ...
    private int getTotalCount(Connection conn, SearchCondition searchCondition) throws SQLException {
        ...
        sql.append("SELECT COUNT(*) ")
           .append("FROM ROOM ro ")
           .append("JOIN REGION r1 ON r1.REGION_ID = ro.REGION_ID ")
           .append("JOIN REGION r2 ON r2.REGION_ID = r1.PARENT_ID ")
           .append("JOIN CERTIFICATION_MASTER cm ON cm.JMCD = ro.JMCD AND cm.YEAR = ro.YEAR AND cm.IMPLSEQ = ro.IMPLSEQ ")
           .append("WHERE 1=1 ");
        if (searchCondition.getSi() != null && !searchCondition.getSi().isEmpty()) {
    ...
    public int insertRoomBoard(RoomBoardDto insertRoomBoardDto) {
        ...
        String sql = "INSERT INTO ROOM_BOARD (room_board_title, room_board_content, user_id, room_board_type, room_id) VALUES (?, ?, ?, ?, ?)";
        try {
            conn = ConnectionPoolHelper.getConnection();
    ...
    public PageResult<JoinRoomUserDto> getJoinRoomMember(int roomId, SearchCondition searchCondition) {
        ...
        sql.append("SELECT u.USER_ID as user_id, ")
           .append("       jr.ROOM_ID as room_id, ")
           .append("       jr.ROOM_TIER as room_tier, ")
           .append("       u.USER_NICKNAME as user_nickname, ")
           .append("       u.USER_PHOTO as user_photo ")
           .append("FROM JOIN_ROOM jr ")
           .append("JOIN \"USER\" u ON u.USER_ID = jr.USER_ID ")
           .append("WHERE jr.ROOM_ID = ? AND u.USER_STATUS = ? ");
        if (searchCondition.getKeyword() != null && !searchCondition.getKeyword().trim().isEmpty()) {
    ...
    public String manageRoomMember(int roomId, int userId, String type) {
        ...
        if("approve".equals(type)){
            sql = "UPDATE JOIN_ROOM jr SET jr.ROOM_TIER = 'MEMBER' WHERE jr.USER_ID = ? AND jr.ROOM_ID = ?";
        } else if("kick".equals(type)){
            sql = "DELETE FROM JOIN_ROOM jr WHERE jr.USER_ID = ? AND jr.ROOM_ID = ?";
        }
    ...
    private int getJoinRoomUserCount(Connection conn ,int roomId) {
        ...
        ResultSet rs = null;
        int totalCount = 0;
        String sql = "SELECT count(*) as count FROM JOIN_ROOM jr JOIN \"USER\" u ON u.USER_ID = jr.USER_ID WHERE jr.ROOM_ID = ? AND u.USER_STATUS = ?";
        try {
            pstmt = conn.prepareStatement(sql);
    ...
    public Map<String, Object> findMyRoomCards(int userId, int offset, int limit) throws Exception {
        ...
        String sql =
            ...
            "JOIN CERTIFICATION_MASTER cm ON cm.jmcd = r.jmcd AND cm.year = r.year AND cm.implSeq = r.implSeq " +
        ...
        try {
            ...
            while (rs.next()) {
                ...
                m.put("parentRegion", rs.getString("parent_region"));
                m.put("childRegion", rs.getString("child_region"));
                m.put("certName", rs.getString("jmName"));
    ...
    public List<Map<String, Object>> findRecentPostsByUser(int userId, int limit) throws Exception {
        ...
        String sql =
            ...
            "       r.ROOM_ID, " +
            "       r.ROOM_TITLE, " +
            "       (SELECT COUNT(*) FROM \"REPLY\" rp WHERE rp.ROOM_BOARD_ID = rb.ROOM_BOARD_ID AND rp.PARENT_REPLY_ID IS NULL) AS REPLY_COUNT " +
            "  FROM ROOM_BOARD rb " +
